...
export const MUSIC_BITRATE = 384_000;
...
export function bitmapSubtitles(pgsClientUnavailable?: boolean): SubtitleProfile[] {
  return pgsClientUnavailable ? BITMAP_SUBTITLES : BITMAP_SUBTITLES_PGS_CLIENT;
}

/** H264 : niveau maximal (51 en Chromium, 52 sous AVFoundation) et 16 trames de référence. */
export function h264Conditions(maxLevel: string): ProfileCondition[] {
  return [
    { Condition: "LessThanEqual", Property: "VideoLevel", Value: maxLevel, IsRequired: false },
 ...
 * conversion qui recompresse l'image entière (cf. `supportedDynamicRanges`).
 */
export function dynamicRangeCondition(ranges: string[]): ProfileCondition {
  return {
    Condition: "EqualsAny",
...
export const AUDIO_PROFILE_6_CHANNELS: CodecProfile = {
  Type: "VideoAudio",
  Conditions: [
 ...
 * `hlsFmp4Profile`) : à vrai, il oblige le serveur à savoir couper hors image
 ...
 */
export function hlsFmp4Profile(
  videoCodec: string,
  audioCodec: string,
  maxChannels = "6",
): TranscodingProfile {
...
export const AUDIO_ONLY_PROFILE: TranscodingProfile = {
  Container: "mp4",
  Type: "Audio",
  AudioCodec: "aac",
  Protocol: "hls",
...
export const TEXT_SUBTITLES: SubtitleProfile[] = [
 ...
 */
export const TEXT_SUBTITLES_HLS: SubtitleProfile[] = [
...
export const BITMAP_SUBTITLES: SubtitleProfile[] = [
 ...
 */
export const BITMAP_SUBTITLES_PGS_CLIENT: SubtitleProfile[] = [
