...
const TO_639_1: Record<string, string> = {
  ...
  endFile: "fi", heb: "he", hin: "hi", hrv: "hr", hun: "hu", ind: "id",
...
const UNDETERMINED = new Set(["und", "mul", "zxx", "qaa", "mis"]);
...
export function normalizeLanguage(raw: string | null | undefined): string | null {
  ...
  if (bare.length === 0 || UNDETERMINED.has(bare)) return null;
  return TO_639_1[bare] ?? bare;
}
