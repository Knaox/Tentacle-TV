...
import { sousTitreParDefaut } from "./useDefaultTracks";
...
describe("sous-titre par défaut", () => {
  ...
  it("garde l'image quand aucun texte n'existe dans la même langue", () => {
    ...
    expect(sousTitreParDefaut(streams, true)).toBe(3);
  });
  ...
  it("ne choisit rien si le fichier ne marque aucun défaut", () => {
    expect(sousTitreParDefaut([st(2, "subrip", "fra")], true)).toBeNull();
    expect(sousTitreParDefaut([], true)).toBeNull();
  });
