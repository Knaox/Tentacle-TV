 ...
 */
export function useImageCassee(src: string | null | undefined) {
  const [etat, setEtat] = useState({ src, cassee: false });
  if (etat.src !== src) setEtat({ src, cassee: false });
  ...
  return {
    ...
    signalerEchec: () => setEtat((e) => (e.src === src ? { ...e, cassee: true } : e)),
  };
}
