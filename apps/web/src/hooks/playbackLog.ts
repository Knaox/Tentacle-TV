 ...
 * console vide.
 */
export function logPlayback(args: {
  verdict: Verdict;
  ...
  const { verdict, videoStream, ranges, transport, directStreamingConfigured, url, directPlay } = args;
  console.warn("[Tentacle:Playback]", {
    mode: verdict.mode,
    reencodage: verdict.videoReencoded,
    ...
    reasons: verdict.reasons.join(",") || "(aucune)",
    ...
    sourceRange: videoStream?.VideoRangeType,
    declaredRanges: ranges.join("|"),
    transport,
