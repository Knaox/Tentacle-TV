...
import { useAuth, useLibraries } from "@tentacle/api-client";
  ...
  const navItems: SidebarItem[] = useMemo(() => {
    ...
    if (libraries) {
      for (const lib of libraries) {
        ...
        items.push({ key: `lib-${lib.Id}`, label: lib.Name, icon });
      }
    }
    items.push(
      { key: "discover", label: "Découvrir", icon: <CompassIcon /> },
      { key: "request", label: "Faire une demande", icon: <PlusIcon /> },
      { key: "requests", label: "Demandes en cours", icon: <ListIcon /> },
      { key: "downloads", label: "Téléchargements", icon: <DownloadIcon /> },
      { key: "support", label: "Aide", icon: <HelpIcon /> },
    );
    return items;
  }, [libraries]);
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-white/5 px-2 py-3 space-y-0.5">
        ...
        <NavButton
          item={{ key: "prefs", label: "Préférences", icon: <LangIcon /> }}
          active={false}
          expanded={expanded}
        ...
        {isAdmin && (
          <NavButton
            item={{ key: "admin", label: "Administration", icon: <AdminIcon /> }}
            active={false}
            expanded={expanded}
        ...
        <NavButton
          item={{ key: "logout", label: "Déconnexion", icon: <LogoutIcon /> }}
          active={false}
          expanded={expanded}
