...
import { compterClicLogo, fermerClassement, classementEstOuvert } from "./logoEggStore";
...
const avancer = (ms: number) => vi.setSystemTime(new Date(vi.getMockedSystemTime()!.getTime() + ms));
...
describe("compteur de clics sur le logo", () => {
  ...
  it("n'ouvre rien avant le quatrième clic", () => {
    for (let i = 0; i < 3; i++) {
      compterClicLogo();
      expect(classementEstOuvert()).toBe(false);
    }
  });

  it("ouvre au quatrième clic rapproché", () => {
    for (let i = 0; i < 4; i++) compterClicLogo();
    expect(classementEstOuvert()).toBe(true);
  });
  ...
  it("repart de zéro quand les clics s'espacent trop", () => {
    ...
    compterClicLogo();
    compterClicLogo();
    avancer(1200); // au-delà de la fenêtre de 900 ms
    compterClicLogo();
    expect(classementEstOuvert()).toBe(false);
  });
  ...
  it("se referme, et le compteur ne garde rien de la fois précédente", () => {
    ...
    for (let i = 0; i < 3; i++) compterClicLogo();
    expect(classementEstOuvert()).toBe(false);
    compterClicLogo();
    expect(classementEstOuvert()).toBe(true);
  });
