...
const TICKET_STYLES: Record<string, { c: string }> = {
  ...
  closed: { c: "bg-fill-soft text-content-quaternary" },
};
...
function TicketDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  if (!ticket) return <p className="text-content-quaternary">{t("common:loading")}</p>;
  ...
  return (<div>
    <button onClick={onBack} className="mb-3 text-sm text-content-tertiary hover:text-content-primary">&larr; {t("common:back")}</button>
    <div className="mb-4 flex items-center gap-3"><h3 className="text-base font-semibold text-content-primary">{ticket.subject}</h3>
    ...
    <div className="max-h-80 space-y-2 overflow-y-auto">{ticket.messages?.map(msg => (
      <div key={msg.id} className={`rounded-lg p-3 ${msg.isAdmin ? "border border-[var(--brand)]/20 bg-[var(--brand-soft)]" : "bg-fill-subtle"}`}>
        <div className="mb-1 flex items-center gap-2 text-xs"><span className={msg.isAdmin ? "font-medium text-[var(--brand-light)]" : "text-content-secondary"}>{msg.username}</span>
          {msg.isAdmin && <span className="rounded bg-[var(--brand-soft)] px-1 py-0.5 text-[10px] text-[var(--brand-light)]">{t("admin:adminBadge")}</span>}
          <span className="text-content-quaternary">{new Date(msg.createdAt).toLocaleString()}</span></div>
        <p className="whitespace-pre-wrap text-sm text-content-secondary">{msg.body}</p></div>))}</div>
...
export function AdminTickets() {
  ...
  return (
    <div className={cls.card}>
      <h2 className="mb-4 text-lg font-semibold text-content-primary">{t("admin:supportTickets")}</h2>
      ...
      {tickets.length === 0 ? <p className="text-sm text-content-quaternary">{t("admin:noTickets")}</p> : <div className="space-y-2">{tickets.map((tk: SupportTicket) => {
        const s = TICKET_STYLES[tk.status]; return (
          <div key={tk.id} onClick={() => setSelId(tk.id)} className="flex cursor-pointer items-center gap-4 rounded-lg border border-line-subtle p-3 hover:bg-fill-subtle">
            <div className="flex-1 min-w-0"><p className="text-sm font-medium text-content-primary truncate">{tk.subject}</p>
              <p className="text-xs text-content-quaternary">{tk.username} — {new Date(tk.updatedAt).toLocaleDateString()}</p>
