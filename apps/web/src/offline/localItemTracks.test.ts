...
describe("mémorisation par contenu", () => {
  ...
  it("écrase l'entrée d'un contenu au lieu de la dupliquer", () => {
    rememberItemTracks("film-1", choix("fre"));
    rememberItemTracks("film-1", choix("jpn"));
    expect(itemTracksFor("film-1")?.audioLang).toBe("jpn");
    const brut = JSON.parse(localStorage.getItem("tentacle_item_tracks_user-1")!) as unknown[];
    expect(brut).toHaveLength(1);
  });

  it("plafonne le cache et évince la plus ancienne", () => {
    for (let i = 0; i < MAX + 25; i++) rememberItemTracks(`ep-${i}`, choix("fre"));
    const brut = JSON.parse(localStorage.getItem("tentacle_item_tracks_user-1")!) as unknown[];
    expect(brut).toHaveLength(MAX);
  ...
  it("garde une entrée re-touchée, même ancienne", () => {
    rememberItemTracks("preferee", choix("jpn"));
    for (let i = 0; i < MAX - 1; i++) rememberItemTracks(`ep-${i}`, choix("fre"));
    // Le cache est plein ; on retouche la doyenne, puis on pousse encore.
    rememberItemTracks("preferee", choix("jpn"));
    for (let i = 0; i < 20; i++) rememberItemTracks(`autre-${i}`, choix("fre"));
  ...
  it("sépare strictement les comptes", () => {
    ...
    rememberItemTracks("film-1", choix("eng"));
  ...
  it("ne fait rien, sans casser, quand personne n'est connecté", () => {
    ...
    expect(() => rememberItemTracks("film-1", choix("fre"))).not.toThrow();
    expect(itemTracksFor("film-1")).toBeNull();
  });
});
