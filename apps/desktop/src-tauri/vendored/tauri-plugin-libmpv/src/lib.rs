    ...
    Builder::new("libmpv")
        ...
        .on_event(|app_handle, run_event| {
            ...
            } = run_event
            {
                ...
                // FORK TENTACLE : ce handler tourne sur le THREAD PRINCIPAL
                // (wndproc). Un lock() bloquant ici a gelé toute l'app le
                // 15.07.2026 (mutex tenu par une commande mpv coincée).
                // Réponse pessimiste si le verrou est occupé : un destroy de
                // label inexistant est un no-op inoffensif.
                let instance_exists = mpv_state.instances.contains_nonblocking(label);
