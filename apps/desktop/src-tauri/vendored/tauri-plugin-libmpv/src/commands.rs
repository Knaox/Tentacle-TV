...
pub(crate) async fn init<R: Runtime>(
    ...
    // FORK TENTACLE : `mpv_wrapper_create` peut bloquer plusieurs secondes
    // (fenêtre, VO, AO) — comme destroy/command, il n'a rien à faire sur un
    // worker async tokio.
    match tauri::async_runtime::spawn_blocking(move || app.mpv().init(mpv_config, &window_label))
        .await
    {
        Ok(result) => result,
