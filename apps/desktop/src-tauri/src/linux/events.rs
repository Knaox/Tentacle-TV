//! Boucle d'évènements mpv (thread dédié) → évènements Tauri.
//!
//! Identique au module macOS (`src/macos/events.rs`) : émet `mpv://event` et
//! `mpv://property-change` — l'adaptateur JS (`mpvLinuxApi.ts`) et
//! `useDesktopPlayer` les consomment à l'identique.
