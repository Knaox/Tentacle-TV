...
export function DiscoverScreen() {
  ...
  const CATEGORIES: { key: Category; label: string }[] = [
    { key: "movies", label: t("discover:movies") },
    { key: "tv", label: t("discover:tvShows") },
    { key: "anime", label: t("discover:anime") },
  ];
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#0a0a0f" }}>
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 56, paddingBottom: 12 }}>
        <Text style={{ color: "#fff", fontSize: 24, fontWeight: "800", marginBottom: 12 }}>{t("discover:title")}</Text>
      ...
      {!isLoading && results.length === 0 && (
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          <Text style={{ color: "rgba(255,255,255,0.4)", fontSize: 15 }}>{t("discover:noContent")}</Text>
        </View>
      )}
