...
import { forgetEncodingSession, readEncodingSession } from "@/lib/transcodeSession";
 ...
 *
 * L'attente sur `readEncodingSession` n'est pas qu'une lecture : elle rend
...
export function TranscodeCleanupSync({ serverUrl }: { serverUrl: string | null }) {
  ...
  useEffect(() => {
    if (doneRef.current || !serverUrl) return;
    doneRef.current = true;

    void (async () => {
      const session = await readEncodingSession();
      if (!session) return;
