...
const STATUS_LABELS: Record<number, { label: string; bg: string; text: string }> = {
  2: { label: "Demandé", bg: "rgba(234,179,8,0.15)", text: "#eab308" },
    ...
    <View style={{ borderRadius: 10, overflow: "hidden", backgroundColor: "#12121a" }}>
      <View style={{ aspectRatio: 2 / 3, backgroundColor: "#1e1e2e" }}>
        ...
        {status === 5 && (
          <View style={{ position: "absolute", top: 6, right: 6, backgroundColor: "rgba(34,197,94,0.9)", paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>
            <Text style={{ color: "#fff", fontSize: 10, fontWeight: "600" }}>Disponible</Text>
          </View>
        )}
      </View>
      <View style={{ padding: 8 }}>
        ...
        {statusInfo ? (
          ...
          >
            <Text style={{ color: "#fff", fontSize: 11, fontWeight: "600" }}>Demander</Text>
          </Pressable>
        )}
      </View>
    </View>
