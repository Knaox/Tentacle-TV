...
/** Ce que la carte édite — la forme commune de la préférence serveur et de la copie locale. */
...
/**
 * La carte d'une bibliothèque sur la page des langues : lecture (badges), puis
 * édition (audio, sous-titres, mode). Elle ne sait pas où part l'enregistrement —
 * le serveur en ligne, la file locale hors ligne — c'est `onSave` qui décide.
 */
export function LibraryPrefCard({ libraryName, pref, onSave, saving }: {
  libraryName: string;
  ...
  const [mode, setMode] = useState<SubtitleMode>("none");
  ...
  const handleSave = useCallback(() => {
    onSave({ audioLang: audio || null, subtitleLang: sub || null, subtitleMode: mode });
    setEditing(false);
  }, [onSave, audio, sub, mode]);
