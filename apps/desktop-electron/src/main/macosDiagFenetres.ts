 ...
 *
 *  - la fenêtre de MPV. `cadreSansLisere` lui rend son `styleMask` titré en
...
import { listerFenetres, nomDeClasse, numeroFenetre } from "./video/objcFenetres";

/** Le contenu d'une `NSString`, ou une chaîne vide. */
function texte(nsstring: unknown): string {
  if (!nsstring) return "";
...
/** Une fenêtre, décrite. */
function decrire(fenetre: unknown, classe: string): string {
  const cadre = msg.rect(fenetre, "frame");
  return (
    `${classe}#${numeroFenetre(fenetre)}` +
    ` titre="${texte(msg.get(fenetre, "title"))}"` +
    ` masque=${msg.count(fenetre, "styleMask")}` +
    ` cadre=${Math.round(cadre.x)},${Math.round(cadre.y)} ` +
    `${Math.round(cadre.width)}x${Math.round(cadre.height)}` +
    ` visible=${msg.bool(fenetre, "isVisible") ? "oui" : "non"}` +
    ` titreVisible=${msg.count(fenetre, "titleVisibility") === 0 ? "OUI" : "non"}` +
    ` barreTransparente=${msg.bool(fenetre, "titlebarAppearsTransparent") ? "oui" : "NON"}`
  );
}
 ...
 */
export function decrireFenetres(): string {
  const fenetres = listerFenetres();
  if (fenetres.length === 0) return "aucune fenetre";
  return fenetres
    .map(([fenetre, classe]) => `  ${decrire(fenetre, classe || nomDeClasse(fenetre))}`)
    .join("\n");
}
