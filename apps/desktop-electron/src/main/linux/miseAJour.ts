...
export type FormatLinux = "appimage" | "deb" | "rpm" | "pacman" | "unknown";
...
function possede(commande: string, args: readonly string[]): Promise<boolean> {
  return new Promise((resoudre) => {
    try {
      const fils = spawn(commande, [...args], { stdio: "ignore" });
      fils.on("error", () => { resoudre(false); });
      fils.on("exit", (code) => { resoudre(code === 0); });
    } catch {
      resoudre(false);
    }
  });
 ...
 */
export async function detecterFormat(): Promise<FormatLinux> {
  if ((process.env["APPIMAGE"] ?? "") !== "") return "appimage";
  const exe = process.execPath;
  if (await possede("pacman", ["-Qo", exe])) return "pacman";
  if (await possede("dpkg", ["-S", exe])) return "deb";
  if (await possede("rpm", ["-qf", exe])) return "rpm";
  return "unknown";
}
...
export function nomSur(nomFichier: string): string {
  const nu = nomFichier.replace(/[/\\]/g, "_").replace(/^\.+/, "_");
  return nu === "" ? "paquet" : nu;
}

/** Télécharge le paquet, vérifie son empreinte, rend son chemin local. */
export async function telecharger(t: Telechargement): Promise<string> {
  const destination = path.join(dossierTemporaire(), nomSur(t.nomFichier));
  const reponse = await fetch(t.url, { redirect: "follow" });
  if (!reponse.ok || reponse.body === null) {
    throw new Error(`téléchargement échoué : ${String(reponse.status)} ${reponse.statusText}`);
  }
  ...
  async function* compter(): AsyncGenerator<Uint8Array> {
    ...
    for await (const bloc of reponse.body as unknown as AsyncIterable<Uint8Array>) {
      empreinte.update(bloc);
      recu += bloc.byteLength;
      if (total > 0) t.progres(recu / total);
      yield bloc;
    }
  }

  await pipeline(compter(), createWriteStream(destination));

  if (t.sha256 !== "") {
    const obtenue = empreinte.digest("hex");
    if (obtenue.toLowerCase() !== t.sha256.toLowerCase()) {
      rmSync(destination, { force: true });
      throw new Error(`SHA256 invalide (attendu ${t.sha256}, obtenu ${obtenue})`);
    }
  }
  return destination;
}
...
function dansLeChemin(binaire: string): boolean {
  const chemins = (process.env["PATH"] ?? "").split(path.delimiter).filter((p) => p !== "");
  return chemins.some((d) => existsSync(path.join(d, binaire)));
}
 ...
 */
function elever(gestionnaire: string, args: readonly string[]): Promise<void> {
  if (!dansLeChemin("pkexec")) {
    return Promise.reject(new Error(
      `pkexec introuvable (installer polkit). À la main : sudo ${gestionnaire} ${args.join(" ")}`,
    ));
  }
  return new Promise((resoudre, rejeter) => {
    const fils = spawn("pkexec", [gestionnaire, ...args], { stdio: "ignore" });
    fils.on("error", (e) => { rejeter(new Error(`pkexec : ${e.message}`)); });
    fils.on("exit", (code) => {
      // 126 = non autorisé, 127 = annulé dans le dialogue polkit.
      if (code === 0) resoudre();
      else rejeter(new Error(`installation annulée ou refusée (pkexec code ${String(code)})`));
    });
  });
}
 ...
 */
function remplacerAppImage(nouveau: string): void {
  const cible = process.env["APPIMAGE"] ?? "";
  if (cible === "") throw new Error("$APPIMAGE absent (l'application n'est pas lancée en AppImage)");
  const sauvegarde = `${cible}.bak`;
  renameSync(cible, sauvegarde);
  try {
    copyFileSync(nouveau, cible);
    chmodSync(cible, 0o755);
  } catch (e) {
    renameSync(sauvegarde, cible);
...
export async function installer(chemin: string, format: FormatLinux): Promise<void> {
  switch (format) {
    case "pacman": return elever("pacman", ["-U", "--noconfirm", chemin]);
    case "deb": return elever("apt-get", ["install", "-y", chemin]);
    case "rpm": return elever("dnf", ["install", "-y", chemin]);
    case "appimage": return Promise.resolve(remplacerAppImage(chemin));
