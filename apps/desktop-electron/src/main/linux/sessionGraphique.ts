...
export type ChoixSession = "auto" | "wayland" | "x11";
export type Montage = "wayland" | "x11";

/** Nom du fichier, dans le dossier de données. */
export const FICHIER_SESSION = "session-graphique.json";

export interface SessionDecidee {
...
function estChoix(v: unknown): v is ChoixSession {
 ...
 */
export function sessionDuBureau(env: Record<string, string | undefined>): Montage | "inconnue" {
 ...
 */
export function deciderSession(
  env: Record<string, string | undefined>,
  choix: ChoixSession,
): SessionDecidee {
  const session = sessionDuBureau(env);
  if (choix === "x11") return { choix, session, ozone: "x11", montage: "x11" };
  if (choix === "wayland") {
    if (session !== "wayland") return { choix, session, ozone: "x11", montage: "x11" };
    return { choix, session, ozone: "wayland", montage: "wayland" };
  }
  return { choix, session, ozone: null, montage: session === "wayland" ? "wayland" : "x11" };
}
 ...
 */
export function lireChoixSession(dossierDonnees: string): ChoixSession {
  const force = process.env["TENTACLE_LINUX_SESSION"];
  if (estChoix(force)) return force;
  try {
    const brut: unknown = JSON.parse(readFileSync(path.join(dossierDonnees, FICHIER_SESSION), "utf8"));
    if (typeof brut === "object" && brut !== null) {
      const v = (brut as { session?: unknown }).session;
      if (estChoix(v)) return v;
    }
  } catch {
