...
export interface EnvironnementDonnees {
  /** `process.platform`. */
...
export function racineDonnees(e: EnvironnementDonnees): string {
  if (e.plateforme !== "linux") return e.appData;
...
export function dossierDonnees(e: EnvironnementDonnees): string {
  return path.join(racineDonnees(e), TAURI_IDENTIFIER);
}
