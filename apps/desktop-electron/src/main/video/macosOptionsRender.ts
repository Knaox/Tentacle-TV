...
import type { ValeurMpv } from "./mpvAllowlist";

/** Options qui n'ont de sens qu'avec une fenêtre à mpv. */
const SANS_OBJET: ReadonlySet<string> = new Set([
  "gpu-api",
  "gpu-context",
  "border",
  "auto-window-resize",
 ...
 */
const BLANC_SDR_NITS = 100;

export function picEdr(): number {
  const potentiel = lireEdr(null).potentiel;
  const nits = Math.round((potentiel > 1 ? potentiel : 1) * BLANC_SDR_NITS);
 ...
 */
export function adapterPourRenderApi(
  options: Readonly<Record<string, ValeurMpv>>,
): Record<string, ValeurMpv> {
  const sortie: Record<string, ValeurMpv> = {};
  for (const [nom, valeur] of Object.entries(options)) {
    if (!SANS_OBJET.has(nom)) sortie[nom] = valeur;
  }
  sortie["vo"] = "libmpv";
  sortie["target-trc"] = "pq";
  sortie["target-prim"] = "display-p3";
  sortie["target-peak"] = picEdr();
  return sortie;
}
