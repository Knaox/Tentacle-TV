...
export function describeMontage(
  ...
  const glue = sameRect(target, video);
  const child = msg.get(mpvWindow, "parentWindow") !== null;
  ...
  const levels = `${msg.int(parent, "level")}/${msg.int(mpvWindow, "level")}`;
  ...
  const fullscreen = host.isFullScreen()
    ? "natif"
    : host.isSimpleFullScreen()
      ? "simple"
      : "non";
  return (
    `cible=${fmt(target)} video=${fmt(video)} calee=${glue ? "oui" : "NON"} ` +
    `electron fenetre=${b.width}x${b.height} page=${c.width}x${c.height} ` +
    `enfant=${child ? "oui" : "NON"} visible=${visible ? "oui" : "NON"} ` +
 ...
 * `macosWindowOptions.ts` serait à revoir, et cette ligne serait le seul
 * endroit où cela se verrait.
 */
export function stateAtDiscovery(window: unknown): string {
  const mask = msg.count(window, "styleMask");
  const promoted = (mask & FULLSCREEN_MASK) !== 0;
  const behaviour = msg.count(window, "collectionBehavior");
  const miniaturized = msg.bool(window, "isMiniaturized");
  return (
    `promueDesLaNaissance=${promoted ? "OUI" : "non"} masque=${mask} ` +
    `comportement=${behaviour} miniaturisee=${miniaturized ? "oui" : "non"}`
  );
}
 ...
 */
function videoState(window: unknown): string {
  const mask = msg.count(window, "styleMask");
  const behaviour = msg.count(window, "collectionBehavior");
  const promoted = (mask & FULLSCREEN_MASK) !== 0;
  return `videoPromue=${promoted ? "OUI" : "non"} masque=${mask} comportement=${behaviour}`;
}
 ...
 */
function display(parent: unknown): string {
