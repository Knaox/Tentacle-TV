 ...
 * comportement correct. Voir `deminiaturiser` dans `macosChildWindow.ts`.
 *
 ...
 */
export function adapterAuPleinEcran(
  options: Readonly<Record<string, ValeurMpv>>,
  host: BrowserWindow,
): Record<string, ValeurMpv> {
  const sortie: Record<string, ValeurMpv> = { ...options };
  if (process.platform !== "darwin" || !host.isFullScreen()) return sortie;
  sortie["window-minimized"] = "yes";
  return sortie;
}
