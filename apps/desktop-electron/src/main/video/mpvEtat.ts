 ...
 */
export function retenir(nom: string, valeur: unknown): void {
  if (valeur === null || valeur === undefined) {
    valeurs.delete(nom);
    return;
  }
  if (typeof valeur === "boolean") {
    valeurs.set(nom, valeur ? "yes" : "no");
    return;
  }
  valeurs.set(nom, String(valeur));
}

/** Dernière valeur connue, ou `null` si mpv n'en a jamais parlé. */
export function valeurConnue(nom: string): string | null {
  return valeurs.get(nom) ?? null;
}
 ...
 */
export function oublierEtat(): void {
  valeurs.clear();
}
