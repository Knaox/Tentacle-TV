...
const COLOR_SPACE = /Metal layer colorspace changed:\s*(\S+)/i;

/** Retient ce qu'un message de journal de mpv dit de la couche Metal. */
export function rememberLog(text: string): void {
  if (ACTIVE.test(text)) inHdr = true;
  else if (INACTIVE.test(text)) inHdr = false;
  const m = COLOR_SPACE.exec(text);
  if (m?.[1] !== undefined) space = m[1];
}

/** La couche Metal est-elle en plage étendue ? `null` si mpv n'a rien dit. */
export function layerInHdr(): boolean | null {
  return inHdr;
}

/** Espace colorimétrique de la couche, tel que mpv le nomme. */
export function layerSpace(): string | null {
  return space;
}

/** Oublie tout — entre deux instances mpv, la couche est recréée. */
export function forgetLayer(): void {
  inHdr = null;
  space = null;
}
