...
const window = (buffer: Buffer) =>
  ({ getNativeWindowHandle: () => buffer }) as unknown as Parameters<typeof nativeHandle>[0];

describe("nativeHandle", () => {
  it("lit les huit octets d'un HWND ou d'un NSView*", () => {
    const b = Buffer.alloc(8);
    b.writeBigUInt64LE(0x7fff_1234_5678n);
    expect(nativeHandle(window(b))).toBe(0x7fff_1234_5678n);
  });
  ...
  it("lit les QUATRE octets d'un numéro de fenêtre X11", () => {
    ...
    expect(nativeHandle(window(b))).toBe(0x600004n);
  });
  ...
  it("rend zéro plutôt que de lever sur un tampon vide", () => {
    ...
    expect(nativeHandle(window(Buffer.alloc(0)))).toBe(0n);
  });
});
