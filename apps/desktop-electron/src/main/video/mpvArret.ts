...
import { nettoyerEtat, poignee, poserAuShutdown, poserPoignee } from "./mpv";
 ...
 *
 * `videoPartie` témoigne de la disparition de la fenêtre vidéo ; sans lui, on se
 * contente d'un nombre de tours fixe, ce qui est moins sûr.
 */
export function arreter(videoPartie?: () => boolean): Promise<void> {
  const ctx = poignee();
  if (!ctx) {
    nettoyerEtat();
  ...
  return new Promise<void>((resolve) => {
    ...
    poserAuShutdown(() => {
      const encore = poignee();
      if (encore) mpvApi().destroyClient(encore);
      poserPoignee(null);
      terminer();
    });
    ...
    const guet = setInterval(() => {
      tours += 1;
      const partie = videoPartie === undefined ? tours >= 10 : videoPartie();
      if (!partie && tours < GUET_MAX) return;
      clearInterval(guet);
      const encore = poignee();
      if (encore) mpvApi().commandAsync(encore, 0, ["quit", null]);
    }, GUET_MS);
  });
}
