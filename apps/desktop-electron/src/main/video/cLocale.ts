...
type Setlocale = (category: number, locale: string | null) => string | null;

let setter: Setlocale | null = null;
 ...
 */
export function setNumericLocaleC(): string | null {
  if (process.platform !== "linux" || unavailable) return null;
  try {
    if (setter === null) {
      const libc = koffi.load("libc.so.6");
      setter = libc.func("char* setlocale(int category, const char* locale)") as unknown as Setlocale;
    }
    const before = setter(LC_NUMERIC, null);
    setter(LC_NUMERIC, "C");
    return before;
  } catch (e) {
