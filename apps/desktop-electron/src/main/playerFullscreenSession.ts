...
import { isFullscreen, noteWindow, WINDOWS_WORKAROUND, leave } from "./fullscreen";
 ...
 * décider (voir `closePlayerSession`). Elle reste commune parce que le contrat
 * avec la page l'est : `player_fullscreen_enter` rend l'état courant sur les deux.
 */
let playerSession: { alreadyFullscreen: boolean } | null = null;
 ...
 * fenêtrée.
 */
export function openPlayerSession(win: BrowserWindow): boolean {
  noteWindow(win);
  const fullscreen = isFullscreen();
  playerSession ??= { alreadyFullscreen: fullscreen };
  return fullscreen;
}
 ...
 */
export function closePlayerSession(win: BrowserWindow): void {
  const session = playerSession;
  playerSession = null;
  // Noté même quand il n'y a rien à faire : `isFullscreen` interroge cette
  // fenêtre-là, et c'est la réponse que `leavePlayerFullscreenScope` rediffuse.
  noteWindow(win);
  applyLinuxExit(win, session);
  if (!WINDOWS_WORKAROUND) return;
  if (session === null) return;
  // Le plein écran était le sien avant le film : il lui appartient.
  if (session.alreadyFullscreen) return;
  if (!isFullscreen()) return;
  leave(win);
}

/** Linux : rend une fenêtre maximisée si le plein écran était celui du film. */
function applyLinuxExit(
  win: BrowserWindow,
  session: { alreadyFullscreen: boolean } | null,
): void {
  const action = decidePlayerExitAction({
    platform: process.platform,
    montage: linuxMontage(),
    windowing: linuxWindowing(),
