...
import {
  ...
  VALEURS_INTERDITES,
} from "./postcss/catalogueChrome53";
import { formaterRefus } from "./postcss/messageCompat";

/**
 * La garde des styles EN LIGNE, symétrique de `gardeCompat`.
 *
 * `gardeCompat` inspecte la feuille produite, et c'est nécessaire. Mais elle ne
...
export interface OccurrenceEnLigne {
  fichier: string;
  ligne: number;
  primitive: string;
  depuis: number;
...
const DEROGATION = /tv-compat-ok\s*:\s*\S+/;
 ...
 */
const PROPRIETES = DECLARATIONS_INTERDITES
  ...
  // traités.
  .filter((entree) => entree.nom !== "backdrop-filter")
  .map((entree) => ({
    ...entree,
    camel: entree.nom.replace(/-([a-z])/g, (_, lettre: string) => lettre.toUpperCase()),
  }));
...
export function gardeStylesEnLigne(): Plugin {
  const occurrences: OccurrenceEnLigne[] = [];
  let derogations = 0;
  ...
  return {
    ...
    transform(code, id) {
      ...
      for (const trouvee of inspecter(code, lignes)) {
        if (aUneDerogation(lignes, trouvee.ligne)) {
          derogations += 1;
          continue;
        }
        occurrences.push({ ...trouvee, fichier: chemin(id) });
      }
      return null;
    },
    ...
    buildEnd(erreur) {
      ...
      if (derogations > 0) {
        console.log(`[garde-styles] ${derogations} dérogation(s) tv-compat-ok utilisée(s)`);
      }
      if (occurrences.length === 0) return;
      this.error(formaterRefus(
        `${occurrences.length} style(s) en ligne trop récent(s) pour le socle du téléviseur :`,
        occurrences.map((o) => `  ${o.primitive} (Chrome ${o.depuis}+) — ${o.consequence}\n      ${o.fichier}:${o.ligne}  ${o.extrait}`),
        [
          "Un attribut `style` échappe aux passes PostCSS comme à gardeCompat :",
...
type Trouvee = Omit<OccurrenceEnLigne, "fichier">;
 ...
 */
function inspecter(code: string, lignes: string[]): Trouvee[] {
  const trouvees: Trouvee[] = [];

  for (const contexte of CONTEXTES) {
    contexte.lastIndex = 0;
    let debut: RegExpExecArray | null;
    while ((debut = contexte.exec(code)) !== null) {
      const region = code.slice(debut.index, debut.index + regionUtile(code, debut.index));
      ...
      const nu = region.replace(/Math\.(max|min)\(/g, "Math·(");
      for (const valeur of VALEURS_INTERDITES) {
        if (!nu.includes(valeur.nom)) continue;
        pousser(trouvees, situer(code, lignes, debut.index, {
          primitive: `${valeur.nom} en style en ligne`,
          depuis: valeur.depuis,
          consequence: valeur.consequence,
        }));
      }
      for (const propriete of PROPRIETES) {
        if (!new RegExp(`\\b(${propriete.camel}|"${propriete.nom}")\\s*:`).test(region)) continue;
        pousser(trouvees, situer(code, lignes, debut.index, {
          primitive: `${propriete.camel} en style en ligne`,
          depuis: propriete.depuis,
          consequence: propriete.consequence,
        }));
      }
    }
  }
 ...
 * affaire.
 */
function regionUtile(code: string, depuis: number): number {
  const PLAFOND = 600;
  let profondeur = 0;
  for (let i = depuis; i < Math.min(code.length, depuis + PLAFOND); i += 1) {
    if (code[i] === "{") profondeur += 1;
    else if (code[i] === "}") {
      profondeur -= 1;
      if (profondeur <= 0) return i - depuis + 1;
    } else if (code[i] === ";" && profondeur === 0) return i - depuis;
  }
  return PLAFOND;
}

function situer(code: string, lignes: string[], position: number, quoi: Omit<Trouvee, "ligne" | "extrait">): Trouvee | null {
...
function aUneDerogation(lignes: string[], ligne: number): boolean {
  return DEROGATION.test(lignes[ligne - 1] ?? "") || DEROGATION.test(lignes[ligne - 2] ?? "");
}

/** N'empile que ce qui est réellement une occurrence. */
function pousser(liste: Trouvee[], trouvee: Trouvee | null): void {
  if (trouvee) liste.push(trouvee);
}

/** Un chemin court, relatif à la racine du dépôt. */
function chemin(id: string): string {
  const marque = id.lastIndexOf("/apps/");
  return marque === -1 ? id : id.slice(marque + 1);
}
