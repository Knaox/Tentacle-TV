...
const sample = (position: number, extra: Partial<PlaybackSample> = {}): PlaybackSample =>
  ({ position, paused: false, ready: 4, error: null, ...extra });
...
describe("lecture qui avance", () => {
  it("ne dit rien tant que la position progresse", () => {
    const { verdicts } = unroll([sample(10), sample(12), sample(14), sample(16), sample(18)]);
    expect(verdicts.every((v) => v === "none")).toBe(true);
  });

  it("ne compte pas une pause voulue comme un gel", () => {
    const stop = Array.from({ length: 8 }, () => sample(42, { paused: true }));
    expect(unroll(stop).verdicts.every((v) => v === "none")).toBe(true);
  });
  ...
  it("ne confond pas un chargement avec un gel", () => {
    ...
    const wait = Array.from({ length: 8 }, () => sample(42, { ready: 2 }));
    expect(unroll(wait).verdicts.every((v) => v === "none")).toBe(true);
  });

  it("ne prend pas une progression minuscule pour de la lecture", () => {
    const ramp = Array.from({ length: SAMPLES_BEFORE_FREEZE + 1 },
      (_, i) => sample(42 + i * (TOLERANCE_PROGRESSION_S / 2)));
    expect(unroll([sample(42), ...ramp]).verdicts).toContain("frozen");
  });
});

describe("lecture figée", () => {
  it("le dit une fois passé le délai de confirmation", () => {
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2719.5));
    const { verdicts } = unroll([sample(2719.5), ...frozen]);
    expect(verdicts.filter((v) => v === "frozen")).toHaveLength(1);
    expect(verdicts.at(-1)).toBe("frozen");
  });
  ...
  it("ne répète pas le constat à chaque relevé", () => {
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE + 6 }, () => sample(2719.5));
    const { verdicts } = unroll([sample(2719.5), ...frozen]);
    expect(verdicts.filter((v) => v === "frozen")).toHaveLength(1);
  });

  it("ne conclut rien avant d'en avoir assez vu", () => {
    const short = Array.from({ length: SAMPLES_BEFORE_FREEZE - 1 }, () => sample(2719.5));
    expect(unroll([sample(2719.5), ...short]).verdicts).not.toContain("frozen");
  });
  ...
  it("signale un gel même quand le lecteur n'a pas d'erreur à montrer", () => {
    ...
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2719.5, { ready: 4 }));
    expect(unroll([sample(2719.5), ...frozen]).verdicts).toContain("frozen");
  });
  ...
  it("traite une coupure réseau comme n'importe quel gel", () => {
    ...
    const cut = Array.from({ length: SAMPLES_BEFORE_FREEZE },
      () => sample(2719.5, { error: MEDIA_ERR_NETWORK }));
    expect(unroll([sample(2719.5), ...cut]).verdicts.filter((v) => v === "frozen")).toHaveLength(1);
  });
});
...
describe("reprise spontanée", () => {
  it("signale le retour de la lecture après un gel", () => {
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2719.5));
    const { verdicts } = unroll([sample(2719.5), ...frozen, sample(2721)]);
    expect(verdicts.at(-1)).toBe("resumed");
  });

  it("ne signale pas de reprise sans gel préalable", () => {
    expect(unroll([sample(10), sample(12), sample(14)]).verdicts).not.toContain("resumed");
  });

  it("rearme la veille pour le gel suivant", () => {
    const frozen = () => Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2719.5));
    const { verdicts } = unroll([
      sample(2719.5), ...frozen(), sample(2721),
      ...Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2721)),
    ]);
    expect(verdicts.filter((v) => v === "frozen")).toHaveLength(2);
  });
  ...
  it("ne prend pas un buffering au milieu du gel pour une reprise", () => {
    ...
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE }, () => sample(2719.5));
    const { verdicts } = unroll([
      sample(2719.5), ...frozen, sample(2719.5, { ready: 2 }), sample(2719.5, { ready: 2 }),
    ]);
    expect(verdicts).not.toContain("resumed");
  });

  it("mémorise l'instant du gel pour qu'on puisse en mesurer la durée", () => {
    const frozen = Array.from({ length: SAMPLES_BEFORE_FREEZE },
      (_, i) => sample(2719.5, { instant: 1000 + i * 2000 }));
    const { state } = unroll([sample(2719.5, { instant: 0 }), ...frozen]);
