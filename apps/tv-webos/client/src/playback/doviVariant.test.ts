...
import { isMasterManifest, doviVariantLine, absoluteVariantUrl } from "./doviVariant";
...
describe("choix de la variante Dolby Vision", () => {
  it("désigne la variante marquée, et non la première venue", () => {
    expect(doviVariantLine(DOVI_MANIFEST)).toBe("main.m3u8?VideoCodec=hevc&PlaySessionId=abc");
  });
  ...
  it("ne se laisse pas prendre par un HDR10 ordinaire", () => {
    ...
    expect(doviVariantLine(PLAIN_MANIFEST)).toBeNull();
  });

  it("rend la main quand la variante n'a pas d'URL", () => {
    // Manifeste tronqué : fabriquer une adresse serait pire que de laisser le
    // téléviseur choisir.
    const truncated = DOVI_MANIFEST.split("\n").slice(0, 2).join("\n");
    expect(doviVariantLine(truncated)).toBeNull();
  });
...
#EXT-X-ENDLIST
`;
    expect(doviVariantLine(sample)).toBeNull();
  });
...
main.m3u8?VideoCodec=hevc
`;
    expect(doviVariantLine(inverse)).toBe("main.m3u8?VideoCodec=hevc");
  });
});
...
describe("résolution en URL absolue", () => {
  ...
  it("résout un manifeste servi par un chemin RELATIF", () => {
    ...
    expect(absoluteVariantUrl("main.m3u8?VideoCodec=hevc", master, PAGE))
  ...
  it("résout aussi un manifeste déjà absolu", () => {
    ...
    expect(absoluteVariantUrl("main.m3u8?b=2", master, PAGE))
  ...
  it("n'hérite pas de la chaîne de requête du manifeste", () => {
    ...
    const u = absoluteVariantUrl("main.m3u8?Tag=def", master, PAGE);
    expect(u).toContain("Tag=def");
    expect(u).not.toContain("api_key=jwt");
  });

  it("rend la main plutôt qu'une adresse fausse", () => {
    expect(absoluteVariantUrl("main.m3u8", "master.m3u8", "pas une URL")).toBeNull();
  });
});

describe("reconnaissance du manifeste maître", () => {
  it("ne s'applique qu'au manifeste maître", () => {
    expect(isMasterManifest("http://h/videos/1/master.m3u8?x=1")).toBe(true);
    // Une lecture directe n'a aucune variante à choisir.
    expect(isMasterManifest("http://h/Videos/1/stream?Static=true")).toBe(false);
    // Une playlist de variante non plus — sans quoi on la re-résoudrait sans fin.
    expect(isMasterManifest("http://h/videos/1/main.m3u8?x=1")).toBe(false);
  });
});
