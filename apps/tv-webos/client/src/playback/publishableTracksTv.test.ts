...
import { pistePubliable } from "./publishableTracksTv";

const piste = (codec?: string): AudioTrack => ({ index: 1, label: "piste", codec });

describe("pistePubliable", () => {
  it("refuse ce qu'aucune génération ne démultiplexe", () => {
    for (const codec of ["truehd", "TrueHD", "mlp", "alac"]) {
      expect(pistePubliable(piste(codec))).toBe(false);
    }
  });
  ...
  it("accepte le DTS — son sort dépend de l'année, pas d'une table absolue", () => {
    ...
    expect(pistePubliable(piste("dts"))).toBe(true);
    expect(pistePubliable(piste("dca"))).toBe(true);
  });

  it("accepte tout ce que la dalle ouvre partout", () => {
    for (const codec of ["aac", "ac3", "eac3", "mp3", "opus", "pcm_s16le", "flac"]) {
      expect(pistePubliable(piste(codec))).toBe(true);
    }
  });
  ...
  it("accepte une piste dont le codec est inconnu", () => {
    ...
    expect(pistePubliable(piste(undefined))).toBe(true);
    expect(pistePubliable(piste(""))).toBe(true);
  });
});
