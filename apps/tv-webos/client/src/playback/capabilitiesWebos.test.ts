...
function materiel(year: number | null, extra: Partial<HardwareTv> = {}): HardwareTv {
  return { year, oled: false, uhd8K: false, ...extra };
}

/** Codecs audio déclarés pour un conteneur donné. */
function audioDe(generation: GenerationWebos, mat: HardwareTv, nom: string): string[] {
  const container = capabilitiesOf(generation, mat).containers.find((c) => c.nom.startsWith(nom));
  return container?.audio ?? [];
}

/** Codecs vidéo déclarés pour un conteneur donné. */
function videoDe(generation: GenerationWebos, mat: HardwareTv, nom: string): string[] {
  const container = capabilitiesOf(generation, mat).containers.find((c) => c.nom.startsWith(nom));
...
describe("dtsSupporte", () => {
  it("suit la chronologie réelle, qui n'est pas monotone", () => {
    expect(dtsSupported(materiel(2017))).toBe(true);
    // LG restreint alors le DTS à l'USB et à l'HDMI — une app n'est ni l'un ni
    // l'autre.
    expect(dtsSupported(materiel(2019))).toBe(false);
    // Licence retirée.
    expect(dtsSupported(materiel(2020))).toBe(false);
    expect(dtsSupported(materiel(2022))).toBe(false);
    // Retiré à nouveau.
    expect(dtsSupported(materiel(2025))).toBe(false);
  });

  it("réintroduit le DTS en 2023-2024 sur les seules dalles OLED", () => {
    expect(dtsSupported(materiel(2023, { oled: true }))).toBe(true);
    expect(dtsSupported(materiel(2024, { oled: true }))).toBe(true);
    // « available in specific models only » : une gamme d'entrée paiera un
    // transcodage audio plutôt qu'une piste silencieuse.
    expect(dtsSupported(materiel(2023))).toBe(false);
  });

  it("refuse quand l'année est inconnue", () => {
    expect(dtsSupported(materiel(null, { oled: true }))).toBe(false);
  });
});

describe("av1Supporte", () => {
  it("attend les gammes 2023", () => {
    expect(av1Supported(materiel(2020))).toBe(false);
    expect(av1Supported(materiel(2022))).toBe(false);
    expect(av1Supported(materiel(2023))).toBe(true);
  });

  it("débloque les modèles 8K quelle que soit l'année", () => {
    expect(av1Supported(materiel(2020, { uhd8K: true }))).toBe(true);
  });

  it("refuse quand l'année est inconnue", () => {
    expect(av1Supported(materiel(null))).toBe(false);
  });
});

describe("capacitesDe — conteneurs", () => {
  it("n'annonce jamais WebM : LG ne le documente dans aucune génération", () => {
    const names = capabilitiesOf(26, materiel(2026)).containers.map((c) => c.nom).join(" ");
  ...
  it("ouvre le MKV, le MP4 et le TS sur toutes les générations", () => {
    for (const generation of [3, 4, 5, 6, 22, 23, 24, 25, 26] as GenerationWebos[]) {
      const names = capabilitiesOf(generation, materiel(null)).containers.map((c) => c.nom);
...
describe("capacitesDe — audio", () => {
  ...
  it("n'annonce JAMAIS le TrueHD — LG ne le liste nulle part", () => {
    ...
    for (const generation of [4, 24, 26] as GenerationWebos[]) {
      const tout = capabilitiesOf(generation, materiel(2024, { oled: true }))
  ...
  it("n'annonce JAMAIS le FLAC dans un conteneur vidéo", () => {
    const capabilities = capabilitiesOf(26, materiel(2026));
...
describe("capacitesDe — rendu client", () => {
  it("ne compte sur WebAssembly qu'à partir de webOS 5", () => {
    ...
    expect(capabilitiesOf(4, materiel(2018)).wasmAvailable).toBe(false);
    expect(capabilitiesOf(5, materiel(2020)).wasmAvailable).toBe(true);
  });

  it("n'ouvre le Dolby Vision en MKV qu'à partir de webOS 25", () => {
    expect(capabilitiesOf(24, materiel(2024)).doviEnMkv).toBe(false);
    expect(capabilitiesOf(25, materiel(2022)).doviEnMkv).toBe(true);
  });
});
