 ...
 */
export function aresUtilisable(racine) {
  if (!racine) return false;
  try {
    return lancerAres(racine, "ares-setup-device", ["--version"]).code === 0;
  } catch {
    return false;
  }
}
 ...
 * Windows — voir `lancerNpm` —, mais c'est ce qui rend l'installation
 ...
 */
export const COMMANDE_NPM =
  "npm install @webos-tools/cli@3 --omit=dev --no-audit --no-fund --loglevel=error";

function lancerNpm() {
  const issue = spawnSync(COMMANDE_NPM, { shell: true, stdio: "inherit", cwd: ATELIER });
...
export function installerAres({ purger = false } = {}) {
  ...
  lancerNpm();
  let racine = localiserAres();
  if (aresUtilisable(racine)) return racine;

  poserLAtelier(true);
  lancerNpm();
  racine = localiserAres();
  if (aresUtilisable(racine)) return racine;
