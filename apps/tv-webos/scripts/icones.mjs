 ...
 *
 * Le logo ne remplit pas sa propre boîte : `SUJET` recadre sur le dessin, sans
 ...
 */
const ICÔNES = [
  { taille: 130, chemin: resolve(IMAGES, "icon-130.png") },
  { taille: 80, chemin: resolve(IMAGES, "icon-80.png") },
  { taille: 400, chemin: resolve(ICI, "../../../store-assets/webos-icon-400.png") },
];

function outilAbsent(nom) {
  return new Error(
...
function lancer(nom, parametres) {
  try {
    execFileSync(nom, parametres, { stdio: ["ignore", "ignore", "inherit"] });
  } catch (erreur) {
    if (erreur.code === "ENOENT") throw outilAbsent(nom);
    throw erreur;
  }
}
...
function corpsDuLogo() {
  ...
  const ouverture = source.indexOf(">", source.indexOf("<svg"));
  const fermeture = source.lastIndexOf("</svg>");
  if (ouverture < 0 || fermeture < 0) {
    throw new Error(`${LOGO} : enveloppe <svg> introuvable`);
  }
  return source.slice(ouverture + 1, fermeture);
}

/**
 * Le logo, mis à l'échelle pour occuper `partHauteur` de la hauteur du canevas
 * et centré sur `centreX` / `centreY`. Le viewBox du `<svg>` imbriqué fait le
 * recadrage, donc la boîte reçue correspond au dessin et à rien d'autre.
 */
function logoPlacé(corps, { canevasLargeur, canevasHauteur, partHauteur, centreY }) {
  const hauteur = canevasHauteur * partHauteur;
  const largeur = (hauteur * SUJET.largeur) / SUJET.hauteur;
  const x = (canevasLargeur - largeur) / 2;
  const y = centreY * canevasHauteur - hauteur / 2;
  const boîte = `${SUJET.x} ${SUJET.y} ${SUJET.largeur} ${SUJET.hauteur}`;
  return (
    `<svg x="${x.toFixed(2)}" y="${y.toFixed(2)}" ` +
    `width="${largeur.toFixed(2)}" height="${hauteur.toFixed(2)}" ` +
 ...
 * butée, les coins sont au-delà. Tout le pourtour vaut donc `BORD`, et la
...
function svgIcône(corps) {
  ...
  <defs>
    <radialGradient id="fondIcone" gradientUnits="userSpaceOnUse" cx="256" cy="256" r="256">
      ...
      <stop offset="100%" stop-color="${BORD}"/>
    </radialGradient>
  ...
  <rect width="${C}" height="${C}" fill="${BORD}"/>
...
const atelier = mkdtempSync(join(tmpdir(), "tentacle-icones-"));
try {
  const corps = corpsDuLogo();

  const sourceIcône = join(atelier, "icone.svg");
  const maître = join(atelier, "maitre.png");
  writeFileSync(sourceIcône, svgIcône(corps), "utf8");
  lancer("rsvg-convert", ["-w", String(MAÎTRE), "-h", String(MAÎTRE), sourceIcône, "-o", maître]);

  for (const { taille, chemin } of ICÔNES) {
    ...
    console.log(`[icones] ${chemin.split("/").slice(-2).join("/")} — ${taille}×${taille}`);
  }
  ...
  console.log(`[icones] splash.png — 1920×1080`);
} finally {
