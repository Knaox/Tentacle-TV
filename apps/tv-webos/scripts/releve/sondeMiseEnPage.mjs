 ...
 *
 * Écrite en ES5 dans une chaîne, comme `sondePage.mjs` et pour la même raison :
 * elle est évaluée par `Runtime.evaluate` dans un moteur qui peut être celui de
 * webOS 4. Elle ne fait que MESURER — l'analyse vit dans `comparerBoites.mjs`,
 ...
 */
export const SONDE_MISE_EN_PAGE = `(function () {
 ...
 * d'autre.
 */
export function routesParDefaut({ bibliotheque, item }) {
  const routes = [{ chemin: "/tv/", nom: "Accueil" }, { chemin: "/tv/settings", nom: "Réglages" }];
  if (bibliotheque) routes.push({ chemin: `/tv/library/${bibliotheque}`, nom: "Bibliothèque" });
  if (item) routes.push({ chemin: `/tv/media/${item}`, nom: "Fiche" });
  return routes;
}
