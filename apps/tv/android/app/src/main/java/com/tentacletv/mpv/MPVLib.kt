 ...
 * rend un handle indépendant. Chaque lecteur crée le sien et le détruit sur un
 * thread à lui (cf. `MpvPlayerView.destroy`) : sans `destroy`, le thread
 * d'événements de libmpv-android resterait bloqué dans mpv_wait_event et sa
 * ref globale épinglerait l'instance — un cœur mpv (décodeur, cache) de plus
 * à chaque lecture. Jamais depuis un rappel mpv (le thread se joindrait
 * lui-même) : sur un thread à nous, le handle déjà oublié de tous.
 */
