...
final class AudioBridge {
    ...
    private static func negotiateLayout(
        ...
        // Modified for Tentacle TV, 2026-09-24 (LGPL-2.1 §2a notice).
        //
        // AAC only writes a channelConfiguration for its "normal" layouts; any
        // other one — the 5.1(side) every DTS / TrueHD decoder hands out — makes
        // FFmpeg's encoder emit a program_config_element, and Apple's decoders
        // refuse AAC carrying a PCE outright: AudioFileOpen fails, AVPlayer
        // reports -16170 on the master and -11829 on the muxed shape. Stock
        // MPVKit has no eac3 encoder, so AAC is the bridge target there and every
        // DTS / TrueHD source failed on tvOS (measured on an Apple TV 4K, tvOS
        // 26.6). Land on the standard layout of the same width instead; the
        // resampler maps side → back.
        if encoder.pointee.id == AV_CODEC_ID_AAC {
            let name: String
            switch source.nb_channels {
            case ...1: name = "mono"
            case 2: name = "stereo"
            case 3: name = "3.0"
            case 4: name = "4.0"
            ...
            try FFmpegError.check(
                av_channel_layout_from_string(&result, name),
                "av_channel_layout_from_string(\(name))"
            )
            return result
        }
