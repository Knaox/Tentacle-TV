 ...
 * sait chercher (`pluginSearch`, @tentacle-tv/shared), lancée à côté de celle
 * de la bibliothèque et jamais devant : la bibliothèque s'affiche dès qu'elle
 * répond, les plugins viennent se ranger dessous.
 *
 ...
 *
 * Commun au web et au mobile : chacun passe SA liste de plugins actifs, sa
 * langue et le nom de repli de la section.
 */
...
import {
  ...
  type ExternalKind, type ExternalSearchResult, type SearchablePlugin,
...
export function useExternalSearch(
  ...
  const { lang, fallbackLabel } = options;
  ...
  const providers = useMemo(
    () => searchProviders(plugins, lang, fallbackLabel).filter((p) => providerAccepts(p, kind)),
    [plugins, lang, fallbackLabel, kind],
  );
