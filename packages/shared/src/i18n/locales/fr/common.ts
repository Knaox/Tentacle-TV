export default {
  save: "Enregistrer",
  cancel: "Annuler",
  delete: "Supprimer",
  edit: "Modifier",
  back: "Retour",
  next: "Suivant",
  previous: "Précédent",
  close: "Fermer",
  confirm: "Confirmer",
  loading: "Chargement...",
  error: "Erreur",
  contentErrorTitle: "Impossible de charger le contenu",
  contentErrorMessage: "Le serveur répond, mais vos médias n'ont pas pu être récupérés. Cela vient souvent d'une interruption passagère.",
  success: "Succès",
  search: "Rechercher",
  searchPlaceholder: "Rechercher...",
  searchMediaPlaceholder: "Film, série, anime...",
  searchMediaLong: "Rechercher un film, une série...",
  noResults: "Aucun résultat",
  noResultsLibrary: "Aucun résultat dans votre bibliothèque",
  typeToSearch: "Tapez pour rechercher dans votre bibliothèque",
  movie: "Film",
  series: "Série",
  anime: "Animé",
  seasons: "saisons",
  seasonsEpisodes: "Saisons & Épisodes",
  similarTitles: "Titres similaires",
  collectionContent: "Contenu de la collection",
  ongoing: "En cours",
  ended: "Terminée",
  play: "Lecture",
  resume: "Reprendre",
  watched: "Vu",
  percentWatched: "{{percent}}% visionné",
  resumeWatching: "Reprendre la lecture",
  nextEpisodes: "Prochains épisodes",
  nextEpisode: "À suivre",
  alreadyWatched: "Déjà visionné",
  latestAdditions: "Derniers ajouts — {{name}}",
  addedEpisodes_one: "+{{count}} épisode",
  addedEpisodes_other: "+{{count}} épisodes",
  page: "Page {{page}} / {{total}}",
  impersonationActive: "Vous naviguez en tant que {{name}}",
  impersonationLeave: "Quitter",
  impersonationLeaving: "Sortie...",
  timeJustNow: "À l'instant",
  timeMinutesAgo: "Il y a {{mins}} min",
  timeHoursAgo: "Il y a {{hours}}h",
  timeDaysAgo: "Il y a {{days}}j",
  ctrlK: "Ctrl+K",
  retry: "Réessayer",
  retrying: "Nouvelle tentative…",
  // L'échec d'UNE rangée, à distinguer d'une bibliothèque sans nouveauté : la
  // seconde ne s'affiche pas, la première se dit et se réessaie.
  rowErrorMessage: "Cette rangée n'a pas pu se charger.",
  send: "Envoyer",
  sending: "Envoi...",
  or: "OU",
  moreInfo: "Plus d'infos",
  details: "Détails",
  fullDetails: "Fiche complète",
  currentEpisode: "Épisode actuel",
  searching: "Recherche en cours...",
  typeMinChars: "Tapez au moins 2 caractères pour rechercher",
  voiceOrType: "Parler ou saisir…",
  space: "Espace",
  clear: "Effacer",
  recentSearches: "Recherches récentes",
  minutesShort: "{{count}} min",
  ratingOutOfTen: "{{rating}} / 10",
  notAvailable: "N/A",
  seeAll: "Tout voir",
  continueLabel: "Continuer",
  latestAdditionsShort: "Derniers ajouts",
  recommendations: "Recommandations",
  searchInLibrary: "Rechercher dans {{name}}...",
  emptyLibrary: "Aucun média dans cette bibliothèque",
  seasonsCount_one: "{{count}} saison",
  seasonsCount_other: "{{count}} saisons",
  offlineTitle: "Oups, le serveur fait une pause !",
  offlineMessage: "Pas de panique, ce n'est pas de votre faute. Le serveur est temporairement injoignable, notre petit poulpe fait tout son possible pour rétablir la connexion.",
  offlineHint: "Cela se résout généralement tout seul. Si le problème persiste, contactez votre administrateur.",
  retryConnection: "Réessayer la connexion",
  offlineLogout: "Se déconnecter",
  changeServer: "Changer de serveur",
  resumeAt: "Reprendre à {{time}}",
  showMore: "Voir plus",
  showLess: "Voir moins",
  cast: "Casting",
  director: "Réalisation",
  writer: "Scénario",
  producer: "Production",
  composer: "Musique",
  crew: "Équipe technique",
  trailer: "Bande-annonce",
  watchTrailer: "Regarder la bande-annonce",
  goToSeries: "Aller à la série",
  extras: "Extras",
  openOnYoutube: "Ouvrir sur YouTube",
  trailerUnavailable: "Lecture YouTube indisponible. Ouvrez le lien externe ci-dessous.",
  trailerUnavailableShort: "Indisponible",
  support: "Support",
  about: "À propos",
  sortRecent: "Récent",
  sortAZ: "A-Z",
  sortYear: "Année",
  itemCount_one: "{{count}} élément",
  itemCount_other: "{{count}} éléments",
  noPlugins: "Aucun plugin disponible",
  librariesTitle: "Bibliothèques",
  libraryTitles_one: "{{count}} titre",
  libraryTitles_other: "{{count}} titres",
  allGenres: "Tous",
  sortDateDesc: "Derniers ajouts",
  sortDateAsc: "Anciens",
  sortTitleAsc: "Titre A→Z",
  sortTitleDesc: "Titre Z→A",
  sortYearDesc: "Année récente",
  sortYearAsc: "Année ancienne",
  sortRatingDesc: "Meilleures notes",
  sortRatingAsc: "Moins bien notés",
  allYears: "Toutes",
  allStatus: "Tous",
  unwatched: "Non vus",
  inProgress: "En cours",
  resultCount_one: "{{count}} résultat",
  resultCount_other: "{{count}} résultats",
  noResultsHint: "Essayez d'autres filtres",
  clearSearch: "Effacer la recherche",
  searchResultCount: "{{count}} titre",
  searchResultCount_other: "{{count}} titres",
  noResultsFor: "Rien ne correspond à « {{query}} » dans {{name}}.",
  tryCorrection: "Essayer « {{term}} »",
  searchEverywhere: "Chercher dans tout Tentacle",
  resetSearchAndFilters: "Effacer la recherche et les filtres",
  myList: "Ma liste",
  addToMyList: "Ajouter à ma liste",
  removeFromMyList: "Retirer de ma liste",
  addToFavorites: "Ajouter aux favoris",
  removeFromFavorites: "Retirer des favoris",
  // États « déjà fait » de la feuille d'appui long du mobile.
  inFavorites: "Dans les favoris",
  inMyList: "Dans ma liste",
  // Action shortcuts (labels courts ≤ 2 mots — toggle via couleur)
  actionFavorite: "Favoris",
  actionMyList: "Ma liste",
  actionShare: "Partager",
  actionWatched: "Vu",
  myFavorites: "Mes favoris",
  filters: "Filtres",
  advancedFilters: "Filtres avancés",
  sortAndFilter: "Trier et filtrer",
  watchStatus: "Visionnage",
  showResultsCount_one: "Voir {{count}} titre",
  showResultsCount_other: "Voir {{count}} titres",
  removeFilterNamed: "Retirer « {{name}} »",
  resetFilters: "Réinitialiser",
  yearFrom: "De",
  yearTo: "À",
  ratingMin: "Note minimum",
  platforms: "Plateformes",
  connectionError: "Erreur de connexion",
  reconnect: "Rejumeler",
  ratingAny: "Toutes",
  sortBy: "Trier par",
  sortOrder: "Ordre",
  sortOrderAsc: "Croissant",
  sortOrderDesc: "Décroissant",
  genres: "Genres",
  favorites: "Favoris",
  allFilter: "Tous",
  moviesFilter: "Films",
  seriesFilter: "Séries",
  emptyWatchlist: "Votre liste est vide",
  emptyFavorites: "Aucun favori",
  emptyWatchlistHint: "Ajoutez des films et séries depuis leur page détail",
  emptyFavoritesHint: "Marquez vos films et séries préférés comme favoris",
  browseLibraries: "Parcourir les bibliothèques",
  invite: "Inviter",
  you: "vous",
  shareMyFavorites: "Partager mes titres likés",
  shareFavoritesLinkDescription: "Toute personne disposant du lien verra tes favoris et tes likes du catalogue — seulement les titres, rien d'autre.",
  sharedFavoritesBy: "Titres likés de {{name}}",
  sharedOffLibraryBadge: "Catalogue",
  shareMyList: "Partager ma liste",
  shareLinkDescription: "Crée un lien vers ta liste. Toute personne avec le lien peut la voir ; connectée, elle peut ajouter des médias à sa propre liste.",
  generateLink: "Générer le lien",
  shareLinkError: "Impossible de générer le lien. Vérifie ta connexion au serveur et réessaie.",
  copyLink: "Copier",
  linkCopied: "Copié !",
  revokeLink: "Révoquer le lien",
  shareAction: "Partager",
  readOnlyList: "Liste partagée",
  sharedListBy: "Liste de {{name}}",
  signInToAdd: "Connecte-toi pour ajouter des médias à ta liste.",
  signIn: "Se connecter",
  shareLinkNotFound: "Ce lien de partage est introuvable ou a été révoqué.",
  addedToMyList: "Ajouté à ma liste",
  adding: "Ajout...",
  backHome: "Retour à l'accueil",
  watchedHidden: "Éléments vus masqués",
  select: "Sélectionner",
  selectAll: "Tout sélectionner",
  deselectAll: "Tout désélectionner",
  selectedCount: "{{count}} sélectionné(s)",
  deleteCount: "Supprimer ({{count}})",
  removeCount: "Retirer ({{count}})",
  toWatch: "À regarder",
  markWatched: "Marquer comme vu",
  markUnwatched: "Marquer comme non vu",
  markSeasonWatched: "Saison vue",
  markSeasonUnwatched: "Saison non vue",
  loadingMore: "Chargement...",
  loadingPlugin: "Chargement de {{label}}...",
  // Écrans volontairement absents du client téléviseur — administration,
  // téléchargements, partage, assistance. Leur route existe encore, un lien
  // interne peut y mener : mieux vaut expliquer qu'afficher une page vide.
  tvIndisponibleTitre: "Indisponible sur le téléviseur",
  tvIndisponibleTexte:
    "Cette section demande un clavier et un écran de près. Ouvrez Tentacle TV sur un ordinateur, un téléphone ou une tablette pour y accéder.",

  // Écran de recherche du téléviseur.
  rechercheTvDictee: "Appuyez sur le micro de la télécommande pour dicter",
  rechercheTvVide: "Saisissez un titre pour lancer la recherche.",
} as const;
