...
- **Une nouvelle barre de navigation** : une capsule qui flotte au-dessus du contenu, avec un onglet par bibliothèque — plus de menu à ouvrir pour atteindre « Films ». Ce qui n'a pas de place passe dans « Plus », qui prend le nom de la page courante quand elle y est rangée
- **La barre se personnalise** : un clic droit sur un onglet le retire — bibliothèques comprises —, et « Personnaliser la barre » (dans « Plus ») permet de tout réordonner en glissant, d'épingler Ma liste, Mes favoris ou une extension, et de revenir à la barre d'origine
...
- **A new navigation bar**: a capsule floating above the content, with one tab per library — no more menu to open to reach "Movies". What does not fit goes into "More", which takes the current page's name when it lives there
- **Make the bar your own**: right-click a tab to remove it — libraries included — and "Customize the bar" (in "More") lets you reorder everything by dragging, pin My List, My Favorites or an extension, and go back to the original bar
...
- **False detections no longer make false promises.** A “post-credit scene” button could appear while the episode carries on under the credits, a “Skip intro” could sit on the epilogue when the opening theme is replayed there, an unusually dense cast crawl could pass for a scene (“Avatar: The Way of Water”) — and Iron Man's very dark scene was never found at all. Each case is fixed, against real measurements
...
## [mac-1.20.10]
### FR
- **Un seul bouton, blanc, partout.** Il compte, il se refuse d'une croix, et il dit où il mène : pendant un générique de fin suivi d'une scène, il propose de sauter jusqu'à la scène, pas jusqu'à l'épisode suivant — et sur le DERNIER épisode d'une série, quand le générique reprend après la scène, il propose alors de terminer la lecture. La croix ne paraît que sur l'image nue — elle arrête le décompte et retire le bouton — sans jamais vous priver du geste : montrez les commandes, il est là
- **Le générique de fin se règle séparément sur les films et sur les séries**, et les films le passent tout seuls d'origine — le bouton mène alors à la scène post-générique. Un film ne se ferme jamais tout seul — et sans scène à rejoindre, il n'affiche plus aucun bouton : le générique se joue, l'écran de fin arrive
...
### EN
- **One button, white, everywhere.** It counts down, it can be refused with a cross, and it says where it leads: during closing credits followed by a scene, it offers to skip to the scene, not to the next episode — and on the LAST episode of a series, when the credits resume after the scene, it then offers to end playback. The cross only appears over the bare picture — it stops the countdown and takes the button away — without ever denying you the gesture: bring up the controls and it is there
- **Closing credits are set separately for films and for series**, and films skip them on their own out of the box — the button then leads to the post-credit scene. A film never closes on its own — and with no scene to reach, it shows no button at all: the credits play out, the end screen arrives
