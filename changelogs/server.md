...
- **La position du lecteur web en transcodage est enfin la vraie** : un transcodage démarré en cours de film (changement de qualité ou de piste, sous-titre incrusté, arrivée dans un visionnage de groupe) pouvait afficher et rapporter une position de 7 à 10 s en retard sur l'image réellement montrée — barre de progression, reprise mémorisée et synchronisation de groupe s'en trouvaient faussées, un membre en transcodage regardant dix secondes plus loin que les autres alors que tout semblait aligné. Le lecteur mesure désormais l'écart réel du flux et le corrige ; une session partie trop loin repart proprement au bon endroit, et un saut hors de la passe d'encodage en cours ouvre une session neuve plutôt que de mélanger deux encodages — c'est ce mélange qui figeait l'image en laissant le son continuer
...
- **The web player's position while transcoding is finally the real one**: a transcode started mid-film (quality or track change, burned-in subtitle, joining a group watch) could display and report a position 7 to 10 s behind the picture actually shown — progress bar, saved resume point and group sync were all off, a transcoding member watching ten seconds ahead of the others while everything looked aligned. The player now measures the stream's real offset and corrects for it; a session that started too far ahead cleanly restarts at the right place, and a seek outside the current encoding pass opens a fresh session instead of mixing two encodings — that mix is what froze the picture while the sound went on
