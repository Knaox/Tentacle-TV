...
- **Web : une nouvelle barre de navigation** : une capsule qui flotte au-dessus du contenu, un onglet par bibliothèque, « Plus » pour le reste et des listes épinglables
...
- **Le tableau de bord de Jellyfin pilote les lecteurs Tentacle** : pour chaque appareil, le serveur tient une connexion Jellyfin et déclare ce que le lecteur sait faire — pause, reprise, arrêt, saut, pistes audio et sous-titres, et messages, qui s'affichent dans l'application, même en pleine lecture
- **« Sessions en direct » dans l'administration** : la vue du tableau de bord de Jellyfin, dédiée à Tentacle — qui regarde quoi, où il en est à la seconde, comment le média arrive (lecture directe, remux ou transcodage : source, flux envoyé, accélération matérielle, raisons), avec pause, reprise, arrêt et message. Les salles Watch Together ont leur section : membres, écart au rythme de la salle, qui transcode quoi — et un message ou un arrêt pour tout le groupe. La liste vient de ce que le serveur reçoit déjà de Jellyfin : la regarder ne lui coûte aucune requête
...
- **Web: a new navigation bar**: a capsule floating above the content, one tab per library, "More" for the rest and pinnable lists
...
- **Jellyfin's dashboard controls Tentacle players**: for each device, the server holds a Jellyfin connection and declares what the player can do — pause, resume, stop, seek, audio and subtitle tracks, and messages, which show up in the app, even during playback
- **"Live sessions" in the administration**: Jellyfin's dashboard view, dedicated to Tentacle — who is watching what, where they are to the second, how the media gets there (direct play, remux or transcoding: source, stream sent, hardware acceleration, reasons), with pause, resume, stop and message. Watch Together rooms have their own section: members, offset from the room, who transcodes what — and a message or a stop for the whole group. The list comes from what the server already receives from Jellyfin: looking at it costs Jellyfin no request
