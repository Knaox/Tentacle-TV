...
- **Web : l'omnibox (⌘K ou Ctrl+K) et une page de résultats** : des résultats à chaque lettre, la correction affichée, la suite du titre proposée en gris, un meilleur résultat qu'on lance d'un clic, et une page par acteur, genre ou studio
- **Web : une nouvelle barre de navigation** : une capsule qui flotte au-dessus du contenu, un onglet par bibliothèque, « Plus » pour le reste. Elle se personnalise : un clic droit retire un onglet — bibliothèques comprises —, et « Personnaliser la barre » réordonne en glissant, épingle listes et extensions, rend la barre d'origine
- **Web : les recherches des bibliothèques, de Ma liste et de Mes favoris** : effacer d'un clic, le nombre de titres trouvés, et quand rien ne répond, l'orthographe proposée par le moteur de Tentacle et la recherche dans tout le serveur
- **Web : Watch Together** : créer un groupe ne lance plus l'invitation d'office — on arrive sur le groupe (au programme, membres), puis on invite une ou plusieurs personnes, dont les invitations restent affichées « en attente » jusqu'à la réponse
- **Web : « Retour » dès l'écran de chargement du lecteur** : un film qui tarde à démarrer se quitte d'un clic ou d'Échap
- **Seules les bibliothèques de films et de séries s'affichent** : Tentacle ne lit que des films et des séries, mais toutes les bibliothèques de Jellyfin apparaissaient — musique, livres, photos, vidéos personnelles, clips, collections, listes de lecture, TV en direct, chaînes d'extension —, chacune avec son onglet, sa rangée et une page vide. Le serveur les écarte désormais de la liste qu'il rend à toutes les applications : restent les films, les séries (animés compris) et les bibliothèques mixtes. Web, bureau, mobile et TV en profitent sans mise à jour ; l'adresse d'une bibliothèque écartée ouvre la page introuvable
- **Fermer un onglet, le navigateur ou l'application arrête la lecture sur le serveur — même après un plantage** : le serveur Tentacle suit désormais chaque lecture du web et du bureau sur la connexion qu'il tient déjà avec eux. Quand un lecteur disparaît (onglet fermé, navigateur quitté, plantage, fin forcée, réseau coupé), il annonce lui-même l'arrêt à Jellyfin, à la dernière position reçue, en quelques secondes — au lieu des cinq minutes de lecture fantôme d'avant
- **Beaucoup moins de requêtes vers Jellyfin** : les lecteurs ne lui envoient plus leur position toutes les dix secondes. Le serveur ne transmet que ce qui change — début, pause, reprise, saut, pistes, fin — et un signe de vie toutes les quatre minutes, le seuil que Jellyfin impose ; en pause sur un flux transcodé, un ping toutes les 40 s garde l'encodage au chaud
- **Le tableau de bord de Jellyfin pilote les lecteurs Tentacle** : pour chaque appareil, le serveur tient une connexion Jellyfin et déclare ce que le lecteur sait faire — pause, reprise, arrêt, saut, pistes audio et sous-titres, et messages, qui s'affichent dans l'application, même en pleine lecture ; un message temporaire affiche son compte à rebours
- **« Sessions en direct » dans l'administration** : la vue du tableau de bord de Jellyfin, dédiée à Tentacle — qui regarde quoi, où il en est à la seconde, comment le média arrive (lecture directe, remux, transcodage audio ou transcodage — distingués sur ce que le serveur calcule vraiment : source, flux envoyé, accélération matérielle, raisons), avec pause, reprise, arrêt et message, chaque commande suivie de l'envoi à l'effet constaté. Chaque appareil affiche son application et sa version (« Tentacle Desktop 1.22.0 », « Tentacle Mobile 1.6.0 · iOS ») : on voit d'un coup d'œil qui n'est pas à jour. Les salles Watch Together ont leur section : membres, écart au rythme de la salle, qui transcode quoi — et un message ou un arrêt pour tout le groupe. La liste vient de ce que le serveur reçoit déjà de Jellyfin : la regarder ne lui coûte aucune requête
- **Aucune session ne peut être détournée** : le serveur ne présente jamais à Jellyfin un identifiant d'appareil venu d'un client, seulement le jeton de l'utilisateur, dont Jellyfin tire lui-même l'appareil. `TENTACLE_SESSION_CHANNEL=off` coupe le canal : les lecteurs reprennent alors leurs reports d'avant
- **Mise à jour requise** : la version minimale de serveur exigée par les applications passe à 1.19.0 (bureau 1.22.0). Les applications qui trouvent un serveur plus ancien le disent par leur bandeau de compatibilité

### EN
- **Tentacle search**: an engine run by the server for the web and desktop apps (`/api/search`). The catalogue is indexed in memory, kept up to date with every addition or removal in Jellyfin, and every answer respects the user's libraries. It answers in a few milliseconds, corrects typos (a letter too many, missing or swapped), understands titles without accents or punctuation, and also finds people, genres and studios — episodes are searched in Jellyfin
- **Web: the omnibox (⌘K or Ctrl+K) and a results page**: results with every letter, the correction shown, the rest of the title suggested in grey, a top result that plays in one click, and a page per actor, genre or studio
- **Web: a new navigation bar**: a capsule floating above the content, one tab per library, "More" for the rest. It is yours to arrange: right-click removes a tab — libraries included — and "Customize the bar" reorders by dragging, pins lists and extensions, and restores the original bar
- **Web: library, My List and My Favorites search**: clear in one click, the number of titles found, and when nothing matches, the spelling suggested by Tentacle's engine and a search across the whole server
- **Web: Watch Together**: creating a group no longer jumps straight to invitations — you land on the group (what is playing, members), then invite one or more people, whose invites stay listed as "waiting" until they get an answer
- **Web: "Back" right from the player's loading screen**: a film that is slow to start can be left with a click or Esc
- **Only movie and TV show libraries are shown**: Tentacle only plays movies and shows, yet every Jellyfin library used to appear — music, books, photos, home videos, music videos, collections, playlists, live TV, plugin channels — each with its tab, its row and an empty page. The server now leaves them out of the list it gives every app: movies, shows (anime included) and mixed libraries remain. Web, desktop, mobile and TV get it without an update; the address of a left-out library opens the not-found page
- **Closing a tab, the browser or the app stops playback on the server — even after a crash**: the Tentacle server now tracks every web and desktop playback over the connection it already holds with them. When a player disappears (tab closed, browser quit, crash, forced quit, network lost), it tells Jellyfin itself that playback stopped, at the last position received, within seconds — instead of the five minutes of ghost playback before
- **Far fewer requests to Jellyfin**: players no longer send it their position every ten seconds. The server only forwards what changes — start, pause, resume, seek, tracks, end — plus a sign of life every four minutes, the threshold Jellyfin enforces; while paused on a transcoded stream, a ping every 40 s keeps the encoder warm
- **Jellyfin's dashboard controls Tentacle players**: for each device, the server holds a Jellyfin connection and declares what the player can do — pause, resume, stop, seek, audio and subtitle tracks, and messages, which show up in the app, even during playback; a temporary message shows its countdown
- **"Live sessions" in the administration**: Jellyfin's dashboard view, dedicated to Tentacle — who is watching what, where they are to the second, how the media gets there (direct play, remux, audio transcode or transcoding — told apart by what the server actually computes: source, stream sent, hardware acceleration, reasons), with pause, resume, stop and message, every command tracked from sending to its observed effect. Every device shows its app and version ("Tentacle Desktop 1.22.0", "Tentacle Mobile 1.6.0 · iOS"): you see at a glance who is not up to date. Watch Together rooms have their own section: members, offset from the room, who transcodes what — and a message or a stop for the whole group. The list comes from what the server already receives from Jellyfin: looking at it costs Jellyfin no request
- **No session can be hijacked**: the server never presents a device identifier coming from a client to Jellyfin, only the user's token, from which Jellyfin derives the device itself. `TENTACLE_SESSION_CHANNEL=off` turns the channel off: players then go back to their previous reports
- **Update required**: the minimum server version required by the apps rises to 1.19.0 (desktop 1.22.0). Apps that find an older server say so through their compatibility banner
...
- **Un interrupteur et un compteur dans Services** : l'analyse audio est active par défaut ; l'administrateur peut la couper, voit l'outil d'empreinte trouvé sur le serveur et ce que la fonction a coûté depuis le démarrage (extraits transcodés, mégaoctets, secondes, verdicts)
- **Image Docker** : le paquet `chromaprint` (fpcalc) rejoint l'image, à côté de yt-dlp — un binaire LGPL invoqué, jamais lié. Hors Docker, un ffmpeg compilé avec chromaprint fait le même travail ; sans l'un ni l'autre, l'analyse se désactive et le dit une fois dans le journal

### EN
- **Episode intros and credits are found by ear**: for an episode no plugin or chapter describes, the server listens to the start and the end of the episode and of its season neighbours — what repeats from one episode to the next is the opening or the ending, found within a few seconds. One analysis per episode, on its first playback, never a background pass: Jellyfin transcodes two short audio excerpts (5 to 10 minutes each, voice quality), cut short as soon as enough has been heard; neighbours already heard are reused, and a twelve-episode season needs about fifteen excerpts. Nothing starts while another viewer is transcoding a video, a server that is too slow puts the feature to rest for an hour, files above 25 Mbit/s are left aside, and the player never waits: the answer arrives a minute later, long before the credits. A scene that goes on after the credits is recognized as such when both neighbours confirm it; at the slightest doubt nothing is set, and plugins always keep the last word
- **A switch and a counter in Services**: audio analysis is on by default; the administrator can turn it off, sees the fingerprint tool found on the server and what the feature has cost since startup (excerpts transcoded, megabytes, seconds, verdicts)
- **Docker image**: the `chromaprint` package (fpcalc) joins the image, next to yt-dlp — an LGPL binary invoked, never linked. Outside Docker, an ffmpeg built with chromaprint does the same job; with neither, the analysis disables itself and says so once in the log

## [1.17.3]
### FR
- **Regarder ensemble, vraiment ensemble** : la synchronisation du visionnage de groupe est refondue. La reprise après une pause est PLANIFIÉE — le serveur fixe l'instant, chaque lecteur s'y cale et démarre en avance de sa propre latence, au lieu de repartir à la réception du message, avec l'écart de réseau et de démarrage de chacun. Après un saut, la salle attend que chacun soit POSÉ sur l'image visée avant de repartir, au même instant — plus de membre qui rattrape pendant dix secondes. La dérive se corrige en continu, cinq fois par seconde, par une variation de vitesse proportionnelle et inaudible (±5 % au plus, hauteur préservée) ; un décalage franc se recale d'un seul saut. La position de chaque lecteur est lue à l'instant même, plus 250 ms plus tôt
- **Le compte à rebours d'un saut est celui de la salle** : quand un membre entre dans un passage que les réglages de l'HÔTE sautent tout seuls, c'est le serveur qui arme UN décompte pour tout le monde — en position de film, une pause le fige — et l'exécute par un saut commun. Chacun voit le même chiffre ; un clic saute tout de suite, une croix l'éteint pour tous, et rembobiner avant le passage le redemande. Un hôte sans réglages enregistrés impose les défauts
- **Les réglages de l'hôte s'appliquent enfin à la salle** : sauts de passages et enchaînement d'épisode gouvernent la séance comme prévu — ils n'arrivaient jamais aux membres, à la création du groupe comme au transfert d'hôte ou quand l'hôte les modifiait
- **L'écart de chaque membre est visible** dans le panneau du groupe : chaque lecteur envoie une balise toutes les cinq secondes, le serveur en tire son avance ou son retard sur la salle (vert sous 50 ms, ambre sous 200 ms, rouge au-delà)
- **Compatible avec les applications d'avant** : un lecteur qui ne connaît pas la nouvelle synchronisation reçoit les mêmes états et se recale comme avant ; il n'est jamais attendu dans une barrière
- **L'image du lecteur web ne se fige plus en transcodage** : revenir en arrière dans une session d'encodage faisait servir par le serveur les fichiers d'une passe précédente à côté de la nouvelle, et le décodeur s'arrêtait à leur frontière — image figée, son maintenu. Un tel saut ouvre désormais une session neuve, le passage de la lecture directe au transcodage se fait sur un élément vidéo neuf, et un décodeur qui s'arrête sans rien dire est relancé d'une recherche
- **Le lecteur web ne coupe plus le son de lui-même** : quand le navigateur refusait de lancer la lecture avec le son, le lecteur relançait en muet et posait un bouton « Appuyer pour le son » — la lecture démarrait silencieuse, parfois sans qu'on comprenne pourquoi. Désormais un refus affiche le bouton de lecture, et le geste lance la lecture avec le son ; le mode muet ne change que lorsque vous le choisissez

### EN
- **Watching together, truly together**: group-watch synchronization is rebuilt. Resuming after a pause is SCHEDULED — the server sets the instant, every player lines up on it and starts early by its own latency, instead of restarting when the message arrives, with everyone's network and start-up lag. After a seek, the room waits until everyone is SETTLED on the target frame before restarting, at the same instant — no more member catching up for ten seconds. Drift is corrected continuously, five times a second, by a proportional and inaudible speed change (±5 % at most, pitch preserved); a large gap is fixed with a single jump. Each player's position is read at that very instant, no longer 250 ms earlier
- **The skip countdown belongs to the room**: when a member enters a segment that the HOST's settings skip automatically, the server arms ONE countdown for everyone — in film position, a pause freezes it — and performs it as a shared jump. Everyone sees the same number; a click skips at once, a cross turns it off for all, and rewinding before the segment asks again. A host without saved settings imposes the defaults
- **The host's settings finally apply to the room**: segment skips and next-episode autoplay govern the session as intended — they never reached the members, at group creation, on host transfer or when the host changed them
- **Each member's offset is visible** in the group panel: every player sends a beacon every five seconds, and the server derives its lead or lag on the room (green under 50 ms, amber under 200 ms, red beyond)
- **Compatible with earlier apps**: a player that does not know the new synchronization receives the same states and realigns as before; it is never waited for in a barrier
- **The web player's picture no longer freezes while transcoding**: seeking backwards within an encoding session made the server serve files from a previous pass next to the new one, and the decoder stopped at their boundary — frozen picture, sound going on. Such a seek now opens a fresh session, switching from direct play to transcoding happens on a fresh video element, and a decoder that stops without a word is nudged by a seek
- **The web player no longer mutes itself**: when the browser refused to start playback with sound, the player retried muted and showed a "Tap for sound" button — playback started silent, sometimes without an obvious reason. A refusal now shows the play button, and the tap starts playback with sound; mute only changes when you choose it

## [1.17.2]
### FR
- **Un plafond de débit pour les téléchargements** : dans la page Téléchargements de l'administration, le serveur se voit fixer la bande passante qu'il consacre aux transferts hors ligne — un plafond pour les clients extérieurs, un autre pour le réseau local (même distinction que la lecture directe, plus une liste d'adresses à traiter comme locales, plafond ou non), chacun pouvant rester illimité. Le débit est partagé équitablement entre les comptes qui téléchargent (6 Mio/s pour deux comptes, c'est 3 Mio/s chacun ; ce qu'un compte lent n'utilise pas revient aux autres, et un compte à deux transferts partage sa part entre eux), et un changement s'applique aussitôt aux transferts en cours, sans redémarrage
- **Le réemballage en MP4 ne demande plus le droit de conversion de média** : il recopie l'image sans la recompresser, seuls les droits de transcodage de lecture le concernent — un compte sans « mode Allégé » peut désormais garder un MKV sur iPhone. Les capacités annoncent `remuxDownloads` et `audioConversion` à part, et la garde de `/api/downloads/light` suit le palier demandé
...
- **Le refus d'un saut atteint tous vos appareils en visionnage de groupe** : le serveur ne transmettait pas « garder l'intro » (ni « masquer l'épisode suivant ») aux autres appareils connectés avec le même compte ; un second appareil laissait son compte à rebours partir et faisait sauter l'intro à toute la salle. Le lecteur web embarqué reçoit les mêmes correctifs que le bureau 1.21.1

### EN
- **A speed cap for downloads**: in the admin Downloads page, the server gets a bandwidth budget for offline transfers — one cap for external clients, another for the local network (same distinction as direct streaming, plus a list of addresses to treat as local, cap or not), each of which may stay unlimited. The rate is shared fairly between the accounts downloading (6 MiB/s for two accounts is 3 MiB/s each; whatever a slow account does not use goes to the others, and an account with two transfers splits its share between them), and changes apply at once to transfers in progress, with no restart
- **MP4 repackaging no longer requires the media-conversion right**: it copies the video without recompressing it, so only playback transcoding rights apply — an account without "Light mode" can now keep an MKV on iPhone. Capabilities advertise `remuxDownloads` and `audioConversion` separately, and the `/api/downloads/light` guard follows the requested tier
...
- **A skip refusal reaches all your devices in group watching**: the server did not pass "keep the intro" (nor "hide next episode") to the other devices signed in with the same account; a second device then let its countdown run and skipped the intro for the whole room. The embedded web player gets the same fixes as desktop 1.21.1
